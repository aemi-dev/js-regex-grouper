...
( () => {
    ...
    const parse = data => {
        ...
        if ( data instanceof ArrayBuffer ) {
            data = ( _ => _.split( /\n/u ) )( new TextDecoder( 'utf-8' ).decode( new Uint8Array( data, 0 ) ) );
        }
        ...
        for ( const [ id, el ] of data.entries() ) {
            ...
            let matched = false;
            let rx;
            const str = typeof el === 'string' ? el : el.text;
    ...
    const answer = ( { match, groups, unmatched } ) => {
        let search;
        let str;
        if ( match === '' ) {
            match = undefined;
        ...
        if ( unmatched === true ) {
            str = _s_.filter( ( { group: { code } } ) => code === 0 );
        }
        else {
            str = _s_;
        }
        if ( match ) {
            ...
            str = str.filter( ( { text } ) => new RegExp( match, 'u' ).test( text ) );
        }
