( ( window, document, undefined ) => {
    ...
    const checkRegExp = () => {
        ...
        for ( const group of groups ) {
            ...
            for ( const regExp of regExps ) {
                ...
                if ( iRv.length > 0 ) {
                    try {
                        nR = iFv.length > 0 ? new RegExp( iRv, iFv ) : new RegExp( iRv );
                    }
                    catch ( error ) {
                        console.warn( error );
                    }
                            ...
                            () => {
                                if ( window.strings.length > 0 ) {
                                    ...
                                    for ( const one of focused || [] ) {
                                        ...
                                        groups[groups.length] = { name, code };
                                    }
    ...
    const arrayItemContentWidthUpdate = () => {
        ...
        document.documentElement.style.setProperty(
            '--item-text-max-width',
            `calc( ${ document.documentElement.offsetWidth > 900 ? 50 : 100 }vw - ${ document.documentElement.offsetWidth > 900 ? 4.5 : 6 }rem - ${ parseInt( i + r + g ) }px )`
        );
    };
    const updateRange = () => {
        ...
        const numberOfPages = Math.floor( length / limit ) + ( length % limit > 0 ? 1 : 0 );
        const i = 0;
        ...
        for ( let i = 0; i < numberOfPages; i += 1 ) {
            if ( page === i ) {
                selectPage.appendChild( ye( { t: 'option', i: [ parseInt( i + 1 ) ], a: { value: i + 1, selected: true } } ) );
            }
            else {
                selectPage.appendChild( ye( { t: 'option', i: [ parseInt( i + 1 ) ], a: { value: i + 1 } } ) );
            }
        }
    ...
    const displayStrings = ( event, data = window.strings, parent = document.getElementsByTagName( 'table' )[0] ) => {
        ...
        let strings = [ ...data ];
        const { prop, order, start, end } = window.view;
        if ( tbody ) {
            tbody.remove();
        }
    ...
    const changeView = ( { prop, order, page, limit, length } ) => {
        ...
        if ( prop === view.prop && page === undefined && limit === undefined ) {
            ...
            order = limit || page ? !order ? 0 : order : !order ? view.order : order;
        }
        ...
        if ( page === undefined ) {
            if ( prop !== view.prop || order !== view.order || limit === undefined && limit !== view.limit ) {
                page = 0;
            }
            else {
                ( { page } = view );
        ...
        else if ( page > Math.floor( length / limit ) + ( length % limit > 0 ? 1 : 0 ) ) {
            page = 0;
        }
        // Check Limit
        if ( limit === undefined ) {
        ...
        window.view = { prop, order, page, start, end, limit };
    ...
    const loadSettings = ( { files, result } ) => {
        if ( result ) {
            try {
                ...
                for ( const group of document.getElementsByClassName( 'group-wrapper' ) ) {
                    group.remove();
                }
                for ( const newGroup of newSettings ) {
            ...
            importReader.readAsText( files[0] );
        }
    };
    importReader.addEventListener( 'loadend', () => {
        loadSettings( importReader );
    } );
    let st;
    let fi;
    let fr;
                            ...
                            const downloadLink = ye( {
                                ...
                                a: {
                                    href: `data:text/plain;charset=utf-8,${ encodeURIComponent( JSON.stringify( window.groups.map( g => {
                                        delete g.count;
                                        for ( const r of g.regExps ) {
                                            r.regExp = { source: r.regExp.source, flags: r.regExp.flags };
                                            delete r.count;
                                        }
                                        return g;
                                    } ) ) ) }`,
                                    download: `ug_${ Date.now() }.settings.json`,
                                    hidden: true
                                }
                            } );
                ...
                i: [ components.regExpGroup(), {
                    ...
                    x: {
                        addEventListener: [ 'click', ( { target } ) => {
                            ...
                            if ( lastGroup ) {
                                lastGroup.insertAdjacentElement( 'afterend', components.regExpGroup() );
                            }
                            else {
                                target.insertAdjacentElement( 'beforebegin', components.regExpGroup() );
                            }
                            checkRegExp();
                        }, false ]
                    }
                ...
                },
                {
                    ...
                    },
                    {
                        i: [ {
                            ...
                            x: {
                                addEventListener: [ 'click', () => {
                                    changeView( { page: window.view.page - 1 } );
                                } ]
                            }
                        },
                        { t: 'label', a: { for: 'select-page' }, i: [ 'Page' ], d: { screenReaderOnly: true } },
                        {
                            t: 'select',
                            id: 'select-page',
                            i: [ { t: 'option', i: [ '1' ], a: { value: 1, selected: true } } ],
                            a: { disabled: true },
                            x: {
                                addEventListener: [ 'input', ( { target: { options, selectedIndex } } ) => {
                                    changeView( { page: options[selectedIndex].value - 1 } );
                                } ]
                            }
                        },
                        {
                    ...
                    },
                    {
                        i: [ {
                            ...
                            x: {
                                addEventListener: [ 'input', ( { target: { options, selectedIndex } } ) => {
                                    changeView( { limit: options[selectedIndex].value } );
                                } ]
                            }
                        },
    ...
    window.addEventListener( 'RegExpChecked', () => {
        ...
        worker.postMessage( { type: 'groups', groups } );
        run.disabled = !criteria();
    } );
    window.addEventListener( 'viewChanged', displayStrings );
    window.addEventListener( 'sendQuery', () => {
        worker.postMessage( window.query );
    } );
